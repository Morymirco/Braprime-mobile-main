...
async function testProfileOperations() {
  ...
  try {
    ...
    // Essayer de récupérer tous les profils (devrait être limité par RLS)
    const { data: allProfiles, error: rlsError } = await supabase
      .from('profiles')
      .select('*');

    if (rlsError) {
      console.log('✅ RLS fonctionne correctement - accès refusé:', rlsError.message);
    } else {
      console.log('⚠️  RLS pourrait ne pas être correctement configuré');
      console.log('Profils accessibles:', allProfiles.length);
    }
